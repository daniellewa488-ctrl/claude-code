...
# Only process emails received within this window — ignores old backlog
_MAX_AGE_HOURS = 72
...
def _is_recent(msg) -> bool:
    """Return True if the email was received within the last _MAX_AGE_HOURS hours."""
    date_str = msg.get("Date", "")
    if not date_str:
        return True  # no date header — assume recent
    try:
...
def fetch_new():
    ...
    'Workflow' received within the last 72 hours that haven't been processed yet.
    """
    results = []
    ...
    try:
        ...
        print(f"[email_reader] Found {len(uids_found)} Workflow email(s) in inbox total")
        ...
        for uid in sorted(uids_found):
            ...
            # Skip emails older than 72 hours — prevents old backlog being processed
            if not _is_recent(msg):
                skipped_old += 1
                # Still mark as processed so we don't keep checking them
            ...
            # Use Message-ID as the dedup key
        ...
        print(f"[email_reader] Result: {new_count} new | {skipped_processed} already processed | {skipped_old} too old (>72h)")
        conn.logout()
